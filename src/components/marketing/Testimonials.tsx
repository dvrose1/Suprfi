          ...
          {testimonials.map((testimonial, i) => (
            <div key={i} className="bg-white p-8 rounded-2xl border border-gray-100 hover:shadow-xl transition-shadow">
              <div className="flex gap-1 mb-4">
                {[1,2,3,4,5].map(star => (
                  <svg key={star} className="w-5 h-5 fill-current text-warning" viewBox="0 0 20 20">
              ...
              <div>
                ...
                <div className="text-sm text-medium-gray">{testimonial.location} &bull; {testimonial.service}</div>
              </div>
            </div>
          ))}
        </div>
